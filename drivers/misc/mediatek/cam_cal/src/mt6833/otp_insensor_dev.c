...
OTP_MAP sc520cs_otp_map_lijinga = {
		...
		.group_info = {
						{"info",
							{
								{0x8826, 0x8834, 0x8835, 0x8836},
								{0x88B1, 0x88BF, 0x88C0, 0x88C1},
							},
						},
						{"awb",
							{
								{0x8837, 0x8846, 0x8847, 0x8848},
								{0x88C2, 0x88D1, 0x88D2, 0x88D3},
							 },
						},
						{"lsc",
							{
								{0x8849, 0x8F94, 0x8F95, 0x8F96},
								{0x88D4, 0x8105, 0x8106, 0x8107},
							},
						},
						{"sn",
							{
								{0x8F97, 0x8FAE, 0x8FAF, 0x8FB0},
								{0x8108, 0x811F, 0x8120, 0x8121},
							},
						},
			},
};
...
static int parse_otp_map_data_sc520cs(OTP_MAP * map, char * data, int temp)
{
	...
	for (i = 0; i < map->group_num; i++) {
		...
		/* in grupe 2. the lsc data Composed of two parts，part one in page2 .another part in page1,but flage&checksum is inpage1 */
		 for (j = 0; j < map->group_addr_info_num; j++) {
			if (!strcmp(map->group_info[i].group_name, "lsc") && j == 1) {
				sc520cs_sensor_otp_init(1, temp);
				LOG_INFO("read grep 2,load page 1");
			}
			ret = map->readFunc(map->group_info[i].group_addr_info[j].group_flag_addr, &group_flag);
		...
		if (!strcmp(map->group_info[i].group_name, "lsc") && j == 1) {    /* There is need cross-page stitching to read, go here */
			sc520cs_sensor_otp_init(2, temp);    /*load page 2 for lsc part one data */
			for (addr = map->group_info[i].group_addr_info[j].group_start_addr; addr <= 0x8FFF; addr++) {
				...
				LOG_INFO("lsc part 1 group: %s, addr: 0x%04x, viraddr: 0x%04x(%04d), data: 0x%04x(%04d) ",
							map->group_info[i].group_name, addr, curr_addr, curr_addr, *data, *data);
			...
			sc520cs_sensor_otp_init(1, temp);    /*load page 1 for lsc part 2 data,0x80E6 is the start addr for part 2 */
			for (addr = 0x80E6; addr <= map->group_info[i].group_addr_info[j].group_end_addr; addr++) {
				ret = map->readFunc(addr, data);	/*read lsc group 2 part 2 data*/
				LOG_INFO("Get SC520CS OTP Data of: %s, addr: 0x%04x, data: 0x%04x", map->group_info[i].group_name, addr, *data);
				if (ret < 0) {
					LOG_ERR("read data error");
				}
				LOG_INFO("lsc part 2 group: %s, addr: 0x%04x, viraddr: 0x%04x(%04d), data: 0x%04x(%04d) ",
							map->group_info[i].group_name, addr, curr_addr, curr_addr, *data, *data);
...
unsigned int sc520cs_read_region(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	for (i = 0; i < OTP_THREAHOLD; i++) {
		...
		ret = parse_otp_map_data_sc520cs(&sc520cs_otp_map_lijinga, &g_otp_buf[IMGSENSOR_SENSOR_IDX_SUB][0], tempotpthreshold);
		if(!ret)
			break;
	}
	if (!ret) {
