...
int disp_ccorr_without_linear = 0;
#endif
#endif
		...
		struct mtk_leds_info *m_leds)
{
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		#ifdef CONFIG_LEDS_MTK_DISP
		ret = of_property_read_u32(child, "disp_ccorr_without_linear",
					&disp_ccorr_without_linear);
		if (ret) {
			disp_ccorr_without_linear = 0;
			pr_info("get disp_ccorr_without_linear property failed\n");
		}
		pr_info("disp_ccorr_without_linear:%d\n", disp_ccorr_without_linear);
		#endif
#endif
